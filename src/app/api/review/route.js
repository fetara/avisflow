...
export async function POST(req) {
  ...
  const customer = await db.customer.findUnique({ where: { id: session.sub }, select: { companyId: true } });
  const minRating = parseInt(await getCompanySetting(customer?.companyId, 'AUTO_APPROVE_MIN_RATING', '0'), 10) || 0;
  ...
  const googleUrl = await getCompanySetting(customer?.companyId, 'GOOGLE_REVIEW_URL', process.env.GOOGLE_REVIEW_URL || '');
